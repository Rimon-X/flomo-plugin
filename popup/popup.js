...
function getCurrentTabInfo() {
  ...
  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
    if (tabs && tabs.length > 0) {
      ...
      console.log('当前页面标题:', pageInfo.title);
...
function useSmartTitle() {
  ...
  console.log('使用智能标题:', smartTitle);
}

// 提交到Flomo
async function submitToFlomo() {
  console.log('开始提交到Flomo');
  // 清除所有错误信息
  clearAllErrors();
  ...
  if (!title) {
    ...
    console.error('提交失败：缺少标题');
    return;
  }
  
  if (!summary && !thoughts) {
    showError('summary-error', '请至少填写摘要或感想中的一项');
    console.error('提交失败：缺少摘要和感想');
    return;
  }
  ...
  try {
    // 发送请求
    console.log('发送请求到Flomo API:', settings.flomoApi);
...
function showError(elementId, message) {
  ...
  errorElement.style.display = 'block';
  console.error(`错误 (${elementId}):`, message);
}
